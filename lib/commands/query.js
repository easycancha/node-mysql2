...
class Query extends Command {
  ...
  start(_packet, connection) {
    ...
    if (this._connection?.config?.addLastQueryToConnection) {
      this._connection.lastQuery = this.sql;
      this._connection.lastQueryActiveTime = Date.now();
      let index = this._connection._pool?._allConnections?._list?.findIndex((c) => c?.threadId == this._connection?.threadId);
      if (index >= 0) {
        this._connection?._pool?._allConnections?._list[index] = this._connection;
      }

      this._connection?._pool.emit('query', this._connection);
    }
